...
const ClientProgressionButtons = ({
  setNewData,
  newData,
  id,
  percentage,
  ...
  const increaseProgression = async (id) => {
    setNewProgression((prevState) => prevState + 10);
    setProgressionId(id);
  };
  const decreaseProgression = async (id) => {
    setNewProgression((prevState) => prevState - 10);
    setProgressionId(id);
  };
  ...
  return (
    <div className={styles.oneProgressionDivButtons}>
      <IconButton
        isDisabled={plusDisable}
        onClick={() => increaseProgression(id)}
        size="xs"
        aria-label="Add to friends"
        icon={<AddIcon />}
      />
      <IconButton
        isDisabled={minusDisable}
        onClick={() => decreaseProgression(id)}
        size="xs"
        aria-label="Add to friends"
        icon={<MinusIcon />}
      />
